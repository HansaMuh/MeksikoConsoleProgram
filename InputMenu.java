...
public class InputMenu 
{
	...
	public void IncreaseSalaryOnNumber()
	{
		...
		for (int I1 = 0; I1 < 3; I1++)
		{
			...
			{
				case 0:
					targetList = Main.Database.stream().filter(target -> "Manager".compareTo(target.Jabatan) == 0).collect(Collectors.toList());
					bonusPercentage = 0.10;
					break;
				case 1:
					targetList = Main.Database.stream().filter(target -> "Supervisor".compareTo(target.Jabatan) == 0).collect(Collectors.toList());
					bonusPercentage = 0.075;
					break;
				case 2:
					targetList = Main.Database.stream().filter(target -> "Admin".compareTo(target.Jabatan) == 0).collect(Collectors.toList());
			...
			if (targetList.size() >= 3)
			{
				...
				System.out.printf("Bonus sebesar %.1lf telah diberikan kepada karyawan dengan id ", (bonusPercentage * 100));
